...
- 从这台 PC 访问 192.168.102.107 得到的是桌上 Zephyr 开发板（`ota-apply ... bkp=1`）：**PC 自己的台架网卡就是 192.168.102.21、跑着 `plc_sim.py`，桌上板接在这个台架网段**，与楼下 IP2 的 PLC 网段物理隔离，没有 IP 冲突（2026-09-11 更正）。同理，从这台 PC 也到不了真 PLC，`pycomm3` 连 .21 得到的是模拟器（CIP 0x08）。

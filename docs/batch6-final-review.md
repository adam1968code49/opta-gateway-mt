...
## 批 6.1（2026-09-08 下午，同日追加）

**触发故障**：批 6 上板 14 分钟后云端断开 95 分钟（12:34–14:09），WiFi 通、PLC 正常、`[HB]` 显示 `cloud=down cloudMs=0` 且无任何云库错误行。
**根因**：`cloud_probe.h` 的门敲的是开机解析一次、永不刷新的 broker IP；`iot.arduino.cc` 是负载均衡，节点轮换后探针永远失败 → `update()` 永不运行 → 云库连自己解析域名的机会都没有。前几批被"卡死→重启→重新解析"盖住，批 6 去掉重启后暴露。另见 `[HB] stall=160006@0`：云端线程曾卡 160 s 于无码段（诊断块 `WiFi.RSSI()` 或心跳打印疑似）。

**改动**（`662afdb` + `95684a6`）：
- 探针缓存地址 10 min 过期、连续 3 次拒绝后重新解析；解析失败保留旧地址继续敲。
- 探针连续被拒 60 s 后每 30 s 放行一次 `update()`（fail-open）——云端已在独立线程，长连接只卡它自己，探针原本保护主线程的理由不再成立。
- WiFi 通、云断连续 15 min → `bootMarkIntentional("cloud offline 15min")` + 自复位；等 `waterOwedL==0`（在飞水量批次不丢），最长宽限到 20 min。WiFi 断时不计时（重启救不了 AP）。
- 位置码 16（心跳打印 + 30 s 诊断块）、17（拍首快照消费 + 状态读取）；云端线程再报 `@0` 只剩"线程没起来"一个含义。
- `[HB]` 增加 `probe=oks/fails failopen=N rr=N`，事后能分清是探针恢复、fail-open 兜住还是 15 min 复位兜住。

**审查（opus）留项**：`bootMarkIntentional` 三线程共用一个 KVStore key 无锁（概率极低，后果仅标记覆盖）；"云连上却零上报"那类半开连接不在 15 min 门的射程内，需要基于"最后一次成功上报时间"的门。

**操作员须知新增**：
1. 上游真断超过 15 分钟，网关每 15 分钟自复位一次直到云回来，`bootReason` 写 `cloud offline 15min`；机器不停，但每次丢约 20–30 s 流量脉冲。
2. 台面上 WiFi 通但 Thing 没配好的板子会进 15 分钟重启循环；`/clear` 断 WiFi 即停止计时。
3. 断网期间 `[HB]` 可能从 3 s 一行变成最长约 50 s 一行（fail-open 的 `update()` 在做 DNS/TLS），看 `probe=` 那一栏判断探针状态。
